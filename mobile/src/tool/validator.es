...
export let checkUsername = (filed, value, callback) => {
	...
	if (!/^[\u4E00-\u9FA5A-Za-z0-9_]+$/.test(value)) {
		return callback(new Error('使用中文英文字母下划线'))
	}
	// 校验通过
	callback();
}
// 邮箱校验
export let checkmobile = (filed, value, callback) => {
	// 密码不能为空
	if (!value) {
		// 提示错误
	...
	if (!/^^\d{6}$$/.test(value)) {
		// 提示错误
		return callback(new Error('邮箱由五位数字组成'))
	}
	// 校验通过
	callback();
}
// 地址校验
export let checkaddress = (filed, value, callback) => {
	// 密码不能为空
	if (!value) {
		// 提示错误
	...
	if (!/^[\u4e00-\u9fa5]{0,}$/.test(value)) {
		// 提示错误
		return callback(new Error('地址必须包含字母和数字'))
	}
	// 校验通过
	callback();
}
