<template>
	<div class="container">
		<div class="address-list">
			<div class="a-address" v-for="(item,index) in addressList" :key="index">
				<div class="left-text" :class="item.isDefault? 'active':''" @click="selectTap(item.id)">
					<div class="name-tel">
						{{item.linkMan}} {{item.mobile}}
					</div>
					<div class="address-box">
						{{item.address}}
					</div>
				</div>
				<div class="right-edit" @click="editAddess(item.id)"> <span class="glyphicon glyphicon-edit"></span></div>
			</div>
		</div>
		<div class="bottom-box">
			<div class="add-btn" @click="addAddess">新增收货地址</div>
		</div>
		<div class="address-hosk" v-show="isShow">
			<el-form ref="addreForm" :model="adderObj" :rules="rules" label-width="100px">
				<!-- 2 为每一个子表单元素，设置校验的字段 -->
				<el-form-item label="用户名" prop="linkMan">
					<el-input placeholder="请输入用户名" v-model="adderObj.linkMan"></el-input>
				</el-form-item>
				<el-form-item label="邮编" prop="mobile">
					<el-input placeholder="请输入邮编" v-model="adderObj.mobile"></el-input>
				</el-form-item>
				<el-form-item label="地址" prop="address">
					<el-input placeholder="请输入地址" type="address" v-model="adderObj.address"></el-input>
				</el-form-item>
				<el-button class="submit-btn" type="success" @click="commit">确定</el-button>
			</el-form>
		</div>
	</div>
</template>

<script>
	import { checkUsername, checkPassword,checkaddress,checkmobile } from '../tool/validator';
	export default {
		data() {
			return {
				isShow:false,
				 labelPosition: 'left',
				 adderObj: {
						linkMan: '',
						mobile:'',
						address: '',
						isDefault:false
				},
				rules:{
					linkMan: [{ validator: checkUsername, trigger: 'blur' }],
					address: [{ validator: checkaddress, trigger: 'blur' }],
					mobile: [{ validator: checkmobile, trigger: 'blur' }],
				},
				addressList: [{
						id: 1,
						linkMan: '小明',
						mobile: '10086',
						address: '北京市西城区动物园',
						isDefault:false
					},
					{
						id: 2,
						linkMan: '小红',
						mobile: '10010',
						address: '北京市海淀区植物园',
						isDefault:true
					},
					{
						id: 3,
						linkMan: '小刚',
						mobile: '10001',
						address: '广东省广州长隆动物园',
						isDefault:false
					
					}
				]
			}
		},
		onLoad() {
			
		},
		methods: {
			selectTap(id) {
				//根据id遍历数组
				this.addressList.forEach((item,index)=>{
					item.isDefault=false;
					if(item.id===id){
						item.isDefault=true;
					}
				})
				let suerinfo = this.addressList.find((item)=>item.isDefault=true);
				//向store中添加用户地址信息
				this.$store.commit('addaddress',suerinfo);
				this.$router.push('/order');
			
			},
			editAddess(id) {
				this.isShow=!this.isShow;
				// let newObj = this.addressList[this.addressList.find((item)=>item.id===id)];
				this.adderObj=this.addressList.find((item)=>item.id===id);
			},
			addAddess() {
				this.adderObj={
					linkMan: '',
					mobile:'',
					address: '',
					isDefault:false
				}
				this.isShow=!this.isShow;
				
			},
			commit(){
				this.$refs.addreForm.validate(valid => {
					console.log(valid,122)
					this.addressList.splice(this.addressList.findIndex(item=>item.id===this.adderObj.id),1);
					this.addressList.push(this.adderObj);
					this.isShow=false;
					// 如果合法
// 					if (valid) {
// 						// 发送请求
// 						this.$http
// 							// post请求
// 							.post('/admin/addaddress', this.adderObj)
// 							// 监听数据返回
// 							.then(({ data }) => {
// 								// 如果登录成功
// 								if (data.errno === 0) {
// 									// 存储用户名
// 									this.$store.commit('updateUsername', data.data.username)
// 								} else {
// 									// 提示错误
// 									alert(data.msg)
// 								}
// 							})
// 					}
				})
			}
		}
	}
</script>

<style type="text/css" lang="scss">
	.container {
		background-color: #F2f2f2;

		.address-hosk {
			position: fixed;
			top: 0;
			bottom: 80px;
			left: 0;
			right: 0;
			background: rgba(0, 0, 0, .4);
			padding:100px 30px 0;
			color: #fff;
			.el-form{
				width: 100%;
				.el-form-item__label{
					color: #fff;
				}
			}
		}
	}

	.address-list {
		width: 100%;
		background-color: #fff;
		margin-top: 20px;
		padding-bottom: 100px;
	}

	.address-list .a-address {
		margin-left: 30px;
		display: flex;
		justify-content: space-between;
		align-items: center;
		padding: 30px 0;
		border-bottom: 1px solid #eee;
	}

	.a-address .left-text {
		width: 610px;
		box-sizing: border-box;
		padding-left: 58px;
	}

	.a-address .left-text.active {
		background: url(data:image/png;base64,) no-repeat left center;
		background-size: 28px auto;
	}

	.a-address .left-text .name-tel {
		margin-bottom: 20px;
	}

	.a-address .left-text .address-box {
		font-size: 24px;
		color: #888888;
		line-height: 36px;
	}

	.a-address .right-edit {
		width: 109px;
		height: 100%;
		padding: 50px 0 50px 58px;
		box-sizing: border-box;
	}

	.bottom-box {
		position: fixed;
		width: 100%;
		bottom: 0;
		font-size: 28px;
		color: #000000;
		border-top: 1px solid #eee;
		height: 100px;
		line-height: 100px;
		background: #fff url(data:image/png;base64,) no-repeat 705px center;
		background-size: 15px auto;
	}

	.bottom-box .add-btn {
		margin-left: 30px;
		padding-left: 70px;
		background: url(data:image/png;base64,) no-repeat left center;
		background-size: 40px auto;
	}
</style>
